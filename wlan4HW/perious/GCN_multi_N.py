...
def clone_model(model, custom_objects={}):
    ...
    return clone
def setup_GCN_model(adj, features, pooling, dense_layer):
...
def setup_DNN_model(conv_layers, conv_kernel_size, pooling, dense_layer):
    input = Input(shape=(NAP*3, NAP*3, 1))
    for layers in conv_layers:
        X = Conv2D(layers, conv_kernel_size, padding='same', activation="relu")(
            input)
...
class Energy():
    ...
    def setupfn(self, model, noise_f):
        ...
        updates = opt.get_updates(params=model.trainable_weights,
                                  loss=self.energy(model.output)+int(noise_f)*tf.random_normal(shape=[1], mean=0., stddev=0))
