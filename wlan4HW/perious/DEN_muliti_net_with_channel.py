...
" 最好进行归一化。"
Interference_matrix = np.zeros([NAP*3, NAP*3])
for i in range(NAP*3):
    for j in range(i+1, NAP*3):
        randomnum = np.random.uniform(0, 1)
        Interference_matrix[i, j] = randomnum
        Interference_matrix[j, i] = randomnum
channel_matrix = np.random.randint(0, Nchannel, size=NAP*3)
outAP = channel_matrix[NAP:]
APoutside_I = np.zeros([NAP,Nchannel])
for j in range(Nchannel):
    index_sage = np.squeeze(np.argwhere(outAP == j)) + NAP
    for i in range(NAP):
        APoutside_I[i,j] = sum(Interference_matrix[i, index_sage])
...
def setupmodel(conv_layers, conv_kernel_size, pooling, dense_layer):
    input = Input(shape=(NAP*3, NAP*3, 1))
...
class Energy():
    ...
    def energy(self,label):
        ...
        energylist = []
        for AP1 in range(NAP):
            energylist.append(tf.reduce_sum(APoutside_I[AP1, :] * label[AP1]))
        energy_outside = tf.reduce_sum(energylist)
        return energy_inside + 0.5 * energy_outside

    def setupfn(self, model, noise_f):
        ...
        updates = opt.get_updates(params=model.trainable_weights,
                                  loss=self.energy(model.output)+int(noise_f)*tf.random_normal(shape=[1], mean=0., stddev=noise))
        train_fn = K.backend.function([model.input, noise], [model.output, self.energy(model.output)],
                                       updates=updates)
        return train_fn

    def compile(self, model_parameter_list):
        N_model = len(model_parameter_list["conv_layers"])
        model = []
        for model_loop in range(N_model):
            ...
            with tf.name_scope(name="model"+str(model_loop)):
                ...
                self.train_fn_list.append(self.setupfn(model[-1], model_parameter_list["noise"][model_loop]))
    ...
    def train(self, times,
        ...
        for loop_time in range(times):
            inputs = self.I + 0.001 * np.random.randn(1, NAP*3, NAP*3, 1)
...
model_parameter_list["conv_layers"] = [[32, 32, 32], [32, 32, 32]]
model_parameter_list["conv_kernel_size"] = [(5, 5), (5, 5)]
model_parameter_list["pooling"] = [0, 0]
model_parameter_list["dense_layer"] = [[128, 128, 128], [128, 128, 128]]
model_parameter_list["noise"] = [0, 1]
assert len(model_parameter_list["conv_layers"]) == Nmodel
assert len(model_parameter_list["conv_kernel_size"]) == Nmodel
assert len(model_parameter_list["pooling"]) == Nmodel
assert len(model_parameter_list["dense_layer"]) == Nmodel
agent.compile(model_parameter_list)
loss, output, record_mean_loss = agent.train(2000)
channel = []
